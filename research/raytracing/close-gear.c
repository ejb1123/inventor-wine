...
void mainCRTStartup(void) {
 ...
 VARIANT doc=invoke(app,L"ActiveDocument",DISPATCH_PROPERTYGET,0,0);VARIANT name=invoke(doc.pdispVal,L"DisplayName",DISPATCH_PROPERTYGET,0,0);if(lstrcmpW(name.bstrVal,L"bevel_gear-1.ipt"))ExitProcess(2);VARIANT skip;VariantInit(&skip);skip.vt=VT_BOOL;skip.boolVal=VARIANT_TRUE;invoke(doc.pdispVal,L"Close",DISPATCH_METHOD,1,&skip);ExitProcess(0);}
