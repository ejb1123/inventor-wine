...
static BOOL WINAPI tracedChoose(HDC dc,const int*a,const FLOAT*b,UINT n,int*f,UINT*c){int attrs[64];int i=0;for(;a[i] && i<60;i+=2){attrs[i]=a[i];attrs[i+1]=a[i]==0x2014?24:(a[i]==0x201b?0:a[i+1]);}attrs[i]=0;char dbg[128];for(int j=0;j<i;j+=2){int test[64];for(int k=0;k<=j+1;k++)test[k]=attrs[k];test[j+2]=0;UINT cnt=0;int pf=0;BOOL ok=realChoose(dc,test,0,1,&pf,&cnt);wsprintfA(dbg,"attr=%x val=%x result=%d count=%u fmt=%d\r\n",attrs[j],attrs[j+1],ok,cnt,pf);out(dbg);}BOOL r=realChoose(dc,attrs,b,n,f,c);char msg[256];wsprintfA(msg,"ChooseARB result=%u count=%u fmt=%d error=%lu\r\n",r,*c,*f,GetLastError());out(msg);return r;}
