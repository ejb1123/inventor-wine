...
void mainCRTStartup(void) {
 ...
 WCHAR *name=L"RayTracing"; DISPID id,put=DISPID_PROPERTYPUT; VARIANT v; VariantInit(&v); v.vt=VT_BOOL; v.boolVal=VARIANT_TRUE; check(IDispatch_GetIDsOfNames(hw.pdispVal,&IID_NULL,&name,1,LOCALE_USER_DEFAULT,&id),"driver property"); DISPPARAMS dp={&v,&put,1,1}; EXCEPINFO e={0}; HRESULT hr=IDispatch_Invoke(hw.pdispVal,id,&IID_NULL,LOCALE_USER_DEFAULT,DISPATCH_PROPERTYPUT,&dp,0,&e,0);if(e.pfnDeferredFillIn)e.pfnDeferredFillIn(&e);char buf[1800];wsprintfA(buf,"scode=%08lx source=%S description=%S\r\n",e.scode,e.bstrSource?e.bstrSource:L"",e.bstrDescription?e.bstrDescription:L"");out(buf);check(hr,"enable GPU ray tracing");
 VARIANT r=invoke(hw.pdispVal,L"RayTracing",DISPATCH_PROPERTYGET,0,0);char b[100];wsprintfA(b,"EnableViewportGPURayTracing=%d\r\n",r.boolVal);out(b);invoke(hw.pdispVal,L"Update",DISPATCH_METHOD,0,0);ExitProcess(0);
}
