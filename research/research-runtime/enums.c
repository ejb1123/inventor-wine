...
void mainCRTStartup(void) {
 ...
 for(UINT i=0;i<ITypeLib_GetTypeInfoCount(lib);i++) { TYPEKIND k; ITypeLib_GetTypeInfoType(lib,i,&k); if(k!=TKIND_ENUM) continue; ITypeInfo *et; ITypeLib_GetTypeInfo(lib,i,&et); BSTR en; ITypeInfo_GetDocumentation(et,MEMBERID_NIL,&en,0,0,0); char b[512]; WideCharToMultiByte(CP_UTF8,0,en,-1,b,512,0,0); if(lstrcmpW(en,L"GraphicsDriverTypeEnum") && lstrcmpW(en,L"GraphicsDriverEnum")) continue; out(b); out("\r\n"); TYPEATTR *ta; ITypeInfo_GetTypeAttr(et,&ta); for(UINT j=0;j<ta->cVars;j++) { VARDESC *v; ITypeInfo_GetVarDesc(et,j,&v); BSTR name; UINT n; ITypeInfo_GetNames(et,v->memid,&name,1,&n); char nm[256]; WideCharToMultiByte(CP_UTF8,0,name,-1,nm,256,0,0); wsprintfA(b,"%s = %ld\r\n",nm,v->lpvarValue->lVal); out(b); } }
 ExitProcess(0);
}
